...
This is the canonical backtest engine for Pasha.
...
5. All 3 modules: A (Wave 3), B (Wave 5), C (Corrective patterns)
"""
...
class BacktestEngine:
    ...
    4. All 3 modules: A (Wave 3), B (Wave 5), C (Corrective patterns)
    """
    ...
    def run(
        ...
        # This happens when consecutive pivot patterns generate opposite signals
        # e.g., H-L-H gives SHORT, then L-H-L gives LONG on same entry_bar
        original_count = len(all_setups)
        all_setups = self._remove_conflicting_entries(all_setups)
        conflicts_removed = original_count - len(all_setups)
    ...
    def _remove_conflicting_entries(self, setups: List[Dict]) -> List[Dict]:
        """
        Remove conflicting entries where LONG and SHORT occur on same bar.
        ...
        Strategy: Remove ALL conflicting setups (ambiguous market signal).
        Alternative: Keep only one direction (not implemented - too arbitrary).
        """
        if not setups:
            return setups

        # Group setups by entry_bar
        by_bar = {}
        for setup in setups:
            bar = setup['entry_bar']
            if bar not in by_bar:
                by_bar[bar] = []
            by_bar[bar].append(setup)

        # Filter out bars with conflicting directions
        filtered = []
        for bar, bar_setups in by_bar.items():
            directions = set(s['direction'] for s in bar_setups)
            if len(directions) == 1:
                # All same direction - keep all setups on this bar
                filtered.extend(bar_setups)
            else:
                # CONFLICT: Both LONG and SHORT on same bar
                # Skip all setups on this bar (market signal is ambiguous)
                pass
...
FixedBacktestEngine = BacktestEngine
FixedBacktestResult = BacktestResult
